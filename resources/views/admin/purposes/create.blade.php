...
<div class="row mb-4">
    <div class="col">
        ...
        <h2 class="h3 text-spiritan">Add New Payment Purpose</h2>
    </div>
</div>
        ...
        <div class="card md-card border-0 p-4">
            <form action="{{ route('purposes.store') }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="name" class="form-control elegant-input @error('name') is-invalid @enderror" value="{{ old('name') }}" placeholder="e.g. Library Fee" required>
                    @error('name') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                
                <div class="mb-3">
                    <label class="form-label text-secondary fw-medium">Description</label>
                    <textarea name="description" class="form-control elegant-input @error('description') is-invalid @enderror" rows="3" placeholder="Optional details about this fee type...">{{ old('description') }}</textarea>
                ...
                <div class="d-grid">
                    <button type="submit" class="btn btn-spiritan py-2">Save Purpose</button>
                </div>
            </form>
        </div>
    </div>
